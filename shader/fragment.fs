...
void main()
{
    ...
    if (use_specular)
        material_specular_color = texture(specular_sampler, frag.uv).rgb * specularity;
    else
        material_specular_color = vec3(1, 1, 1)*specularity;
    ...
    if (use_normal)
    {
        n = normalize(texture(normal_sampler, vec2(frag.uv.x, frag.uv.y)).rgb*2.0 - 1.0);
        l = normalize(frag.light_direction_tbn);
        E = normalize(frag.eye_direction_tbn);
    }
    else
    {
        n = normalize(frag.normal_mv);
        l = normalize(frag.light_direction_mv);
        E = normalize(frag.eye_direction_mv);
    }
    vec3 R = reflect(-l, n);
